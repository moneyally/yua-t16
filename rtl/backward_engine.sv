...
//
// mxu_bf16_16x16 을 인스턴스화하므로 그 모듈의 성격(손으로 만든 FP32 행동 모델,
// 가산기 256개)을 그대로 물려받는다. scripts/synth_gate.sh 에서 yosys 시간 초과로
// **미측정** 상태다 ("합성 불가"가 아니다 — 최종 판정은 Vivado).
//
// CLAUDE.md 2절이 이 모듈을 격리 대상에서 명시적으로 제외했기 때문에
// rtl/behavioral/ 로 옮기지 않았다. 학습(역전파) 경로는 docs/DESIGN.md 10절에서
// 범위 밖이므로, DR1 v1 (Q1.15 델타룰) 작업 중에는 손대지 않는다.
// DR1 v2 착수 시 다시 판단한다.
// -----------------------------------------------------------------------------
