...
## 5.2 감사 이후 — W1-2 게이트를 세우면서 추가로 드러난 것 (2026-09-10 같은 날)

`scripts/synth_gate.sh` 를 만들어 돌리자마자 **테스트 261개가 통과하는 동안 숨어 있던
합성 결함 6개**가 나왔다. 전부 `docs/BUGS.md` 에 명령 출력과 함께 기록했다.

| # | 모듈 | 문제 | 상태 |
|---|---|---|---|
| BUG-001 | `desc_fsm_v2` → `g2_ctrl_top` | fault 디스크립터가 `DESC_DONE` 완료 IRQ 를 올린다 (`IRQ_PENDING=0x21`) | 재현·파형 확보, **미수정** |
| BUG-002 | `cdc_fifo` | `initial` 블록이 **포트 리스트 안**에 있어 컴파일 자체가 안 됐다 | 수정 |
| BUG-003 | `mxu_bf16_16x16` | 함수 안 경계 없는 `while` — 합성 불가 | 수정 |
| BUG-004/5 | `desc_fsm_v2`, `g3_desc_fsm` | `fault_code_r` 를 두 `always_ff` 가 구동 (다중 드라이버) | 수정 |
| BUG-006 | `wgt_sram`, `gemm_core` | 시뮬 전용 `=== 1'bx` X 가드가 **합성에서 로직을 삭제** — wgt_sram 0 cells, gemm_core 로직 95% 소멸 | 수정 |

**BUG-006 이 이 감사 전체에서 가장 중요한 항목이다.** 고치기 전 상태로 실물에 올렸다면
가중치 SRAM 이 없고 GEMM 결과가 0 으로 기록되는 칩이 나왔다. 시뮬레이션은
`` `ifdef COCOTB_SIM `` 이 켜진 채 돌기 때문에 끝까지 정상으로 보였다.

### 추가 분류 변경 (rtl/behavioral/ 로 이동)

`real` 을 쓰지 않지만 **하드웨어가 아닌** 모듈 2개를 추가로 격리했다:

| 모듈 | 근거 |
|---|---|
| `kvc_core` | `kv_store[0:3][0:3][0:63][0:3][0:1][0:15]` = 6차원 배열 = **2 Mbit 플립플롭**. yosys `std::bad_alloc` (6GB). 아무 데도 인스턴스화되지 않음. `docs/DESIGN.md` §8 "v1 미사용". |
| `mxu_bf16_128x128` | `tile_acc[0:7][0:7][0:15][0:15]` = **524,288 플립플롭**. 아무 데도 인스턴스화되지 않음. `CLAUDE.md` 2절 "이름이 실체보다 크다". |

`real` 을 쓰지 않는다고 합성 가능한 것이 아니다 — 이 둘은 `optimizer_unit` 과 같은
범주(행동 모델)이며, 손으로 만든 float 이나 거대 배열로 그것을 감췄을 뿐이다.

### 여전히 측정 못 한 것 (시간 초과, 합성 불가가 아님)

`backward_engine`, `g2_ctrl_top`, `g2_protob_top`, `g3_int_top`, `mxu_bf16_16x16`,
`gemm_int4_synth`, `vpu_core_synth` — 모듈당 기본 예산(240s) 안에 yosys 가 끝내지 못했다.
`mxu_bf16_16x16` 은 FP32 가산기 256개라 20분+ 걸린다. **"느려서 못 끝냄"이지 "합성 불가"가 아니다.**
...
| `mac_pe`, `mac_array` 합성 가능 | ⚠️ **`mac_pe`만 사실.** `mac_array`는 sv2v 없이는 yosys가 읽지 못함 (196,352 셀 — sv2v 경유 확인). W1-2 에서 sv2v 전처리를 게이트에 넣어 해결 |
