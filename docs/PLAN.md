...
- [x] W1-1 `rtl/behavioral/` 생성, `real` 사용 7개 모듈 이동 (`collective_engine, gemm_int4, loss_scaler, moe_router, optimizer_unit, vpu_core, vpu_fp16_utils`). 이들을 참조하는 G3 top·테스트벤치도 `rtl/behavioral/`, `tb/behavioral/`로 이동. README에 "행동 모델(합성 불가)" 섹션으로 분리 표기.
- [x] W1-2 `scripts/synth_gate.sh` 작성: `rtl/*.sv`(behavioral 제외) 전체를 yosys로 합성, 실패 시 비-0 종료. `grep` 금지 토큰 검사 포함. 이 스크립트가 통과하는 상태를 첫 커밋으로.
- [x] W1-3 `verilator --lint-only -Wall` 을 합성 대상 전 모듈에 돌리고, 경고 목록을 `docs/LINT.md`에 기록. 고치지는 않는다(기록만).
- [x] W2-1 README 전면 재작성: 현재 상태표(제어 평면 ✔ / mac_array ✔ / PCIe 스텁 / DDR 없음 / 학습 경로 = 행동 모델), 실행 가능한 명령만 기재. "MPW ready", "LLM inference accelerator", "training" 문구 제거. 새 한 줄: "INT8 16×16 외적 누산 타일 + 검증된 제어 평면. 다음 목표: 델타룰 헤드 (docs/DESIGN.md)".
- [x] W2-2 `done_pulse` 버그 재현: 실패하는 최소 테스트벤치 하나를 `tb/tb_gemm_core_done.py`로 만들고, VCD를 덤프해 `docs/BUGS.md`에 "어느 사이클에 어떤 신호가 기대와 다른지" 한 문단 기록. 이번 주에 고치지 않아도 된다. **재현 + 파형 기록이 완료 기준.**
- [x] W2-3 `docs/LOG.md` 시작.
...
- [x] W8 **1,000토큰 × 시드 3개 비트 일치** (`tb_dr1_harness_rtl.py` R9). `SAT_EVENT`/`CLAMP_EVENT` 를 g2_ctrl_top 트레이스 링에 연동. **d=16 완료.** **d=64 도 확인** — 같은 RTL 을 `PARAM_D=64 PARAM_SCRATCH=8192` 로 돌려 20토큰 비트 일치, 462사이클 (`tb/tb_dr1_d64.py`). 통합 빌드는 면적 때문에 d=16 유지.
...
- [x] W9 `tools/orbit_device.py` 에 `delta_init/delta_step/delta_dump` + 스크래치 MMIO 접근 추가. `CocotbBackend` 로 Python HAL → RTL E2E.
- [x] W10 `tb/tb_dr1_host_e2e.py`: 호스트 스택(OrbitDevice→CocotbBackend→g2_ctrl_top)으로 1·10토큰 비트 일치 + 스크래치 왕복 + fault 전달 + 오류 주입. **3단계 종료 기준 충족** (`dev.delta_step()` → RTL → 골든 일치).
