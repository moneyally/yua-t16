...
### 이어서 — **아무도 안 보고 있던 질문**: 긴 시퀀스에서 상태가 발산하는가

다른 모든 검증은 "RTL == 골든" 을 본다. 그건 **골든이 진짜 델타룰인가**에 아무
답도 주지 않는다. 그런데 이 레포의 북극성은 **상태가 오래 머무는 것**이 전제다 —
Q1.15 상태가 200토큰 뒤에 다른 것이 되어 있으면 비트 정확도는 의미가 없다.

이 검사는 `sim/golden/deltarule.py` 의 `__main__` 안에만 있었다. 손으로
`python3 sim/golden/deltarule.py` 를 돌려야 보였다 — **pytest 에 없었다.**

측정하고 답을 얻었다 (`docs/DESIGN.md` 6.5절 표):

| α 대역 | 100토큰 | 400 | 800 | 1600 | 3200 |
|---|---|---|---|---|---|
| 0.80–0.99 | 2.1 | 2.0 | 1.8 | 1.7 | — |
| 0.999–0.9999 | 7.0 | 9.1 | 11.8 | 10.6 | — |
| **α = 최대(0.999969)** | **7.3** | **10.9** | **11.3** | **11.7** | **10.6** |

**드리프트는 유계다.** α 를 표현 가능한 최대값(사실상 망각 없음)으로 고정해도
400→3200토큰에서 ~11 LSB 로 평탄하다. 풀스케일 32768 대비 **0.034%**.
이유는 α 감쇠만이 아니다 — α≈1 에서도 평탄하므로, 실제로는 **양자화 오차의
부호가 무작위라 상쇄**되기 때문이다 (유계 상태 안의 랜덤워크).

- `sim/golden/deltarule.drift_series()` 신설 (체크포인트별 기록 — 끝점 하나로는
  "수렴하는가" 에 답할 수 없다).
- `tests/test_golden_drift.py` 신설 (6 테스트). 절대 상한뿐 아니라 **기울기**를
  단언한다. "드리프트가 0 이 아닌가" 도 본다 — 0 이면 두 경로가 같은 걸 계산하는
  것이고 파일 전체가 무의미해진다.
- **이 테스트가 무는지 확인했다**: α 를 무시해 안 잊게 만드는 뮤턴트
  (`state_never_decays`)를 넣으면 죽는다. 반올림 방식 변경은 **안 죽는다** —
