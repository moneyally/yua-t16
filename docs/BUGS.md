...
| W11 | `tb_axi4_master_adapter` 의 슬레이브 모델이 **두 번째 버스트를 안 받았다** (M5 무한 대기) | 버스트를 처리하며 `awready` 를 내린 뒤 **되돌리지 않았다**. 읽기 쪽(`run_read`)에는 `arready=1` 복원이 있었고 쓰기 쪽에만 없었다. 4KB 경계를 넘는 쓰기만 버스트가 2개라 M5 에서만 드러났다 |
| W11 | `tb_wdog_timer` 가 **호출 사이에 rising edge 를 하나 흘렸다** (T4·T6 이 1사이클 일찍 터짐) | `run_cycles` 가 매 호출 앞에서 다시 falling edge 에 정렬했다. 그 정렬이 rising edge 를 하나 건너뛰는데 **모델은 그 사이클을 안 센다** → DUT 만 1사이클 앞선다. 정렬은 `start()` 에서 한 번만 한다 |
| W11 | `tb_wdog_timer` T3 의 **기대값 자체가 틀렸다** | RTL 도 모델도 맞았다. 리셋 직후 `win_cnt` 는 PERIOD 가 아니라 0 이라 첫 창이 1024 사이클이다 — KICK 없이 켜면 그렇다. 테스트가 호스트와 다르게(`wdog_ctrl_word` 는 KICK 을 항상 싣는다) 켠 것이 원인. T7 로 이 동작을 못 박았다 |

앞의 셋은 원인이 같다: **등록된 신호를 rising edge 직후에 읽었다.**
규칙은 `docs/BUGS.md` BUG-008a/b 에서 이미 나왔다 — **falling edge 에서 샘플링한다.**

넷째·다섯째는 다른 종류다: **모델의 상태 복원 누락**과 **모델과 DUT 의 사이클 어긋남**.
둘 다 테스트가 "RTL 이 틀렸다"고 거짓말했다. 읽기 경로에 있던 복원이 쓰기 경로에
없었으므로, 규칙은 **양쪽 채널을 나란히 놓고 같은 모양인지 본다** — 한쪽에만 있는
줄이 있으면 그게 의심 지점이다. 그리고 **참조 모델을 호출마다 새로 만들지 않는다**;
DUT 는 이어지는데 모델만 0 에서 출발하면 엉뚱한 곳에서 불일치가 난다.

여섯째는 또 다른 종류다: **RTL 도 모델도 맞는데 기대값이 틀렸다.** 테스트가 호스트와
다른 순서로 레지스터를 건드렸다. 규칙: **테스트도 호스트 헬퍼를 쓴다**
(`wdog_ctrl_word()`). 비트를 손으로 쓰면 호스트와 테스트가 갈라지는 날이 온다.
